...
- [ ] Write language admission RED: `reference.language === undefined` remains admitted (Danish-source default); explicit `reference.language === "da"` remains admitted; explicit `reference.language === "en"` returns `null` with zero fetches and zero recorder calls; any other unsupported explicit language performs zero recorder calls; no unsupported explicit language can be returned as Danish fallback.
- [ ] Write sourceVariant admission RED: `reference.sourceVariant === undefined` remains admitted; explicit `reference.sourceVariant === "translation-en"` returns `null` with zero fetches and zero recorder calls; any other unsupported explicit `sourceVariant` performs zero recorder calls; no explicit DK `sourceVariant` is synthesized.
- [ ] The later production fix for these admission gates must be a gate at the provider boundary and must not alter transport, LexDania parsing, identity semantics, currentness semantics, Task 7 behavior, or the existing `LawReference`, `LawSection`, `language`, or `sourceVariant` types.
- [ ] Run RED with native compile + focused test.
