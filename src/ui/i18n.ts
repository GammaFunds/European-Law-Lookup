...
import { FINLAND_UI_STRINGS, UI_LANGUAGE_CODES, UI_STRINGS } from "./i18nCatalog";
...
export interface UiStrings extends UiPresentationStrings {
  ...
  defaultFiTextLanguage?: string;
  defaultFiTextLanguageDescription?: string;
  finlandAcceptedInputFormats?: string;
  finlandInputExamples?: string;
  finlandScopeNote?: string;
  finlandConsolidatedNotice?: string;
...
export function getUiStrings(languageCode: unknown): UiStrings & { defaultJurisdiction: string; defaultJurisdictionDescription: string } {
  ...
  return { ...UI_STRINGS[language], ...FINLAND_UI_STRINGS[language], ...DEFAULT_JURISDICTION_STRINGS[language] };
}
